...
<?php
 ...
 ?>
                    ...
                    <div class="col-md-10 bg-white">
                        ...
                        <?php
                        $select_others = $con->Query("SELECT * FROM others");
                        while ($row = mysqli_fetch_array($select_others)) {
                          ...
                          $select_profit = $con->query("SELECT * FROM profit_others WHERE No = '$No'");
                          $count_profit = mysqli_num_rows($select_profit);
                          $count = $count_profit > 0;
                          // Adding profit
                          if (!$count) {
                            // echo "Here";
                            $add_profit = $con -> Query("INSERT INTO `profit_others`(`Date`, `No`, `Designation`, `Unit`, `Qty`, `Profit_unit`, `Profit`, `Status`)
                                                                     VALUES ('$date' , '$No', '$item', '$unit', '$qty', '$Profit_unit', '$Profit', '$status')
                                                                     ") or die(mysqli_error($con));
                          }
                          else {
                            while ($row_profit = mysqli_fetch_array($select_profit)) {
                              $update_profit = $con->query("UPDATE profit_others SET Qty='$qty',Profit_unit='$Profit_unit',Profit='$Profit' WHERE No = '$No'") or die(mysqli_error($con));
                              $delete_profit = $con->query("DELETE FROM profit_others WHERE No != '$No'");
                            }
                          }
                         }
                          ?>
                	...
                	<div class="col-md-12">
                      ...
                      <div class="col-md-12">
                       <div class="well shadow-sm bg-white p-4">
                        ...
                        <table class="table table-sm ">
                            ...
                            <td><?php echo $row['Profit'] ;?></td>
                				...
                				<?php endwhile ?>
                        <?php
                          $make_sum = $con->Query("SELECT SUM(Profit) AS profit_sum FROM others");
                        ?>
                       ...
                       <?php endwhile ?>
                        ...
                        <tr colspan="3">
                          <center>
                              <a href="others.php"><button class="btn btn-sm teal text-white">Return Back</button></a>
                          </center>
                        </tr>
                			</p>
                		</div>
